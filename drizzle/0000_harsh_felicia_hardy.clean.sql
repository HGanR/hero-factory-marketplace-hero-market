...
CREATE TABLE `trust_debt_instruments` (
	...
	`debtInstrumentType` enum('bond','private_placement','secured_trust','revenue') NOT NULL,
	`securitiesExemption` enum('reg_d_506b','reg_d_506c') NOT NULL,
	`bondNumber` varchar(80) NOT NULL,
	...
	`seniority` enum('senior','subordinated') NOT NULL DEFAULT 'senior',
...
CREATE TABLE `trusts` (
	...
	`firmAddress` text,
	`firmPhone` varchar(80),
	`firmEmail` varchar(320),
	`trustType` enum('revocable_living_trust','irrevocable_trust','testamentary_trust','special_purpose_trust'),
	`governanceDocs` json NOT NULL DEFAULT (JSON_ARRAY()),
	`constitutionSubtype` enum('none','dao_token_voting','church','unincorporated_association','pma') NOT NULL DEFAULT 'none',
	`governancePackage` enum('none','bylaws_standard','bylaws_foundation','bylaws_religious','bylaws_family_office') NOT NULL DEFAULT 'none',
...
ALTER TABLE `clients` ADD CONSTRAINT `clients_userId_marketplace_users_id_fk` FOREIGN KEY (`userId`) REFERENCES `marketplace_users`(`id`) ON DELETE no action ON UPDATE no action;
CREATE INDEX `idx_bondholder_register_instrument` ON `trust_bondholder_register` (`debtInstrumentId`);
CREATE INDEX `idx_bondholder_register_trust` ON `trust_bondholder_register` (`trustId`);
CREATE INDEX `idx_debt_authority_checks_instrument` ON `trust_debt_authority_checks` (`debtInstrumentId`);
CREATE INDEX `idx_debt_authority_checks_trust` ON `trust_debt_authority_checks` (`trustId`);
CREATE INDEX `idx_debt_collateral_instrument` ON `trust_debt_collateral` (`debtInstrumentId`);
CREATE INDEX `idx_debt_disclosures_instrument` ON `trust_debt_disclosures` (`debtInstrumentId`);
CREATE INDEX `idx_trust_debt_instruments_trustId` ON `trust_debt_instruments` (`trustId`);
CREATE INDEX `idx_trust_debt_instruments_status` ON `trust_debt_instruments` (`instrumentStatus`);
