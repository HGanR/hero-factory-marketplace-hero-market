...
def main():
    if len(sys.argv) != 3:
        print("Usage: generate_bill_of_exchange_pdf.py <input_json_path> <output_pdf_path>", file=sys.stderr)
        sys.exit(2)
    ...
    title = payload.get("title") or "Bill of Exchange"
    ...
    y -= 0.18 * inch
    c.drawString(x, y, f"Bill Number: {payload.get('billNumber','')}")
    y -= 0.18 * inch
    ...
    y -= 0.18 * inch
    c.setFont("Helvetica", 10)
    c.drawString(x, y, f"Drawer: {payload.get('drawer','')}")
    y -= 0.16 * inch
    c.drawString(x, y, f"Drawee: {payload.get('drawee','')}")
    y -= 0.16 * inch
    c.drawString(x, y, f"Payee: {payload.get('payee','')}")
    y -= 0.25 * inch
    ...
    y -= 0.18 * inch
    c.setFont("Helvetica", 10)
    c.drawString(x, y, f"Amount: {payload.get('amount','')}")
    y -= 0.16 * inch
    c.drawString(x, y, f"Issue Date: {payload.get('issueDate','')}")
    y -= 0.16 * inch
    c.drawString(x, y, f"Due Date: {payload.get('dueDate','')}")
    y -= 0.16 * inch
    c.drawString(x, y, f"Jurisdiction: {payload.get('jurisdictionState','')}")
    y -= 0.18 * inch
    y = draw_multiline(c, f"Place of Issue: {payload.get('placeOfIssue','')}", x, y)
    y = draw_multiline(c, f"Place of Payment: {payload.get('placeOfPayment','')}", x, y)
    y -= 0.1 * inch
