...
import {
  buildTrustPacketPreviewText,
  isTrustFulfillmentNote,
  parseFulfillmentOrderIdFromTrustPayload,
  parseTrustFulfillmentNoteFields,
} from "@/lib/fulfillment/fulfillment-trust-deliverable-draft-parse";
import {
...
async function findLatestTrustPacketNote(db: Db, clientId: string) {
  const rows = await db
    ...
    .limit(30);
  return rows.find((r) => isTrustFulfillmentNote(r.note)) ?? null;
}

export async function linkTrustPacketToFulfillmentDeliverable(
  db: Db,
  input: {
    adminUserId: number;
    approvalId: string;
  ...
  const orderId = parseFulfillmentOrderIdFromTrustPayload(input.payload);
  if (!orderId) return;

  const order = await loadTrustOrderForAdmin(db, { orderId, adminUserId: input.adminUserId });
  if (!order) return;
  ...
  await auditFulfillmentExecutiveAction(db, {
    ...
    toolName: "fulfillment.trust.deliverable.link_packet",
    actionType: "trust_packet_linked",
...
export async function loadTrustDeliverableDraftForOrder(
  db: Db,
  input: { orderId: string; adminUserId: number }
): Promise<TrustFulfillmentDeliverableDraftDto | null> {
  const order = await loadTrustOrderForAdmin(db, input);
  ...
  if (!noteRow) {
    const fallback = await findLatestTrustPacketNote(db, order.clientId);
    if (fallback) {
      noteRow = fallback;
      noteId = fallback.id;
    }
  ...
  const parsed = noteRow ? parseTrustFulfillmentNoteFields(noteRow.note) : null;
  const canReview =
    linked && deliverable.ownerReviewStatus === "pending" && order.pipelineStage === "owner_review";
  ...
  return {
    ...
    packetType: parsed?.packetType ?? deliverable.artifactType,
    previewText: noteRow ? buildTrustPacketPreviewText(noteRow.note) : null,
    hasLegalDisclaimer: parsed?.hasLegalDisclaimer ?? false,
...
export async function approveTrustDeliverableForRelease(
  db: Db,
  input: { orderId: string; adminUserId: number }
): Promise<TrustDeliverableReviewResult> {
  const order = await loadTrustOrderForAdmin(db, input);
  if (!order) {
  ...
  if (!deliverable?.artifactRef?.trim()) {
    return {
      ok: false,
      httpStatus: 409,
      code: "packet_not_linked",
      message: "No trust packet is linked for owner review yet.",
    };
  }
  ...
  await db.transaction(async (tx) => {
    ...
    await insertFulfillmentOrderEvent(tx, {
      ...
      payloadJson: {
        ...
        action: "trust_packet_approved_for_release",
  ...
  await auditFulfillmentExecutiveAction(db, {
    adminUserId: input.adminUserId,
    toolName: "fulfillment.trust.deliverable.approve_packet",
    actionType: "trust_packet_approved_for_release",
    targetType: "fulfillment_deliverable",
    targetId: deliverable.id,
  ...
  return {
    ...
    message:
      "Trust packet approved for release internally. No trust execution, apply, or client delivery in Slice 1.",
  };
}

export async function requestTrustDeliverableRevision(
  db: Db,
  ...
  if (!deliverable?.artifactRef?.trim()) {
    return {
      ...
      httpStatus: 409,
      code: "packet_not_linked",
      message: "No trust packet is linked for revision.",
    };
  }
  ...
  await db.transaction(async (tx) => {
    ...
    await insertFulfillmentOrderEvent(tx, {
      ...
      payloadJson: {
        deliverableId: deliverable.id,
        clientNoteId: deliverable.artifactRef,
        action: "trust_packet_revision_requested",
        revisionNote: note,
      },
    });
  });
  ...
  await auditFulfillmentExecutiveAction(db, {
    adminUserId: input.adminUserId,
    toolName: "fulfillment.trust.deliverable.request_revision",
    actionType: "trust_packet_revision_requested",
    targetType: "fulfillment_deliverable",
    targetId: deliverable.id,
  ...
  return {
    ...
    message: "Revision requested. Propose a new trust packet when ready — no trust apply or execution.",
  };
}
