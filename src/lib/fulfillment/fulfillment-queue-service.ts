...
import {
  ...
  type FulfillmentQueueListResultDto,
  type FulfillmentQueueOrderSummaryDto,
...
export type ListFulfillmentQueueInput = {
  adminUserId: number;
  limit?: number;
  stage?: string | null;
...
export async function listWebsiteFulfillmentQueueForAdmin(
  db: Db,
  input: ListFulfillmentQueueInput
): Promise<FulfillmentQueueListResultDto> {
  ...
  if (!orderIds.length) {
    await auditFulfillmentExecutiveAction(db, {
      ...
      toolName: "fulfillment_queue.list",
    ...
    return {
      ...
      meta: {
        ...
        approvalFilter,
        primaryService: FULFILLMENT_PRIMARY_SERVICE_WEBSITE,
      },
    };
  }
  ...
  const summaries: FulfillmentQueueOrderSummaryDto[] = [];
