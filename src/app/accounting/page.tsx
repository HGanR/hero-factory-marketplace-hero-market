...
import EnhancedAccountingSystem from "@/components/EnhancedAccountingSystem";
import MobileWalletButton from "@/components/MobileWalletButton";

// Token gate constants (ported from OLDSITE/pages/accounting.tsx)
...
export default function AccountingPage() {
  ...
  // Soft app-session gate (matches the rest of this app: marketplace localStorage + admin flag)
  useEffect(() => {
    try {
  ...
  const checkTokenBalance = useCallback(async () => {
    ...
    try {
      ...
      // Use latest state after possible manual scan
  ...
  // Re-run balance when manual scan finishes updating state
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center">
        <div className="max-w-md mx-auto p-8 text-center">
          ...
          {!isConnected ? (
            <div className="space-y-4">
              <p className="text-slate-300">
                Please connect your wallet with sufficient TROO tokens on Polygon.
              </p>
            ...
            <>
              <p className="text-slate-300 mb-4">
                You need at least {REQUIRED_TROO_AMOUNT.toLocaleString()} TROO tokens to access the accounting platform.
              </p>
              <div className="bg-slate-800 rounded-lg p-4 mb-6 text-left">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${displayBalance >= REQUIRED_TROO_AMOUNT ? "text-green-400" : "text-red-400"}`}
                    >
          ...
          <div className="text-xs text-gray-400 mt-3 text-left bg-slate-800 p-4 rounded">
            <p>• Contract: {TROO_POLYGON_CONTRACT}</p>
            <p>• Network: Polygon (Chain ID 137)</p>
            <p>• Required: {REQUIRED_TROO_AMOUNT.toLocaleString()} TROO</p>
          </div>
        </div>
      </div>
    );
  ...
  try {
    if (typeof document !== "undefined") {
      document.title = "TroothHurtz Accounting";
    }
  } catch {
    // ignore
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100">
      <div className="fixed right-4 top-4 z-50 flex items-center gap-2">
        <button
          ...
          className="rounded-lg border border-slate-800 bg-slate-950 px-4 py-2 text-sm font-semibold text-slate-100 shadow hover:bg-slate-900/60"
        >
          Dashboard
        </button>
        <button
        ...
        <button
          onClick={() => router.push("/accounting/reporting")}
          className="rounded-lg border border-slate-800 bg-slate-950 px-4 py-2 text-sm font-semibold text-slate-100 shadow hover:bg-slate-900/60"
        >
          REPORTING
        </button>
