...
import { completeExecutiveOperationalTask } from "@/lib/executive-agent/operational-task-service";
...
export async function POST(req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    const result = await completeExecutiveOperationalTask(db, {
      adminUserId,
      taskId: id,
    ...
    return NextResponse.json({ error: "COMPLETE_FAILED", message: msg }, { status: 500 });
  }
}
