...
const BodySchema = z.object({
  method: z.enum(["wet_sign_attestation", "e_sign", "wallet_sig"]).default("wet_sign_attestation"),
  custodianIdentity: z.string().max(255).optional(),
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ trustId: string; certificateId: string }> }) {
  ...
  await db.transaction(async (tx) => {
    await tx.insert(securityEvents).values({
      ...
      certificateId,
      eventType: "POSSESSION_ACKNOWLEDGED",
      actorUserId: userId,
      actorRole: "trustee",
      payloadJson: JSON.stringify({
        method: body.method,
        custodianIdentity: body.custodianIdentity ?? null,
        note: body.note ?? null,
      }),
    } as any);

    // Convenience fields for UI; ledger is source-of-truth.
    await tx
      .update(securityCertificates)
      .set({
        possessionAcknowledgedAt: acknowledgedAt,
        possessionAcknowledgedMethod: body.method,
      } as any)
      .where(eq(securityCertificates.id, certificateId));
  });

  return NextResponse.json({
    trustId,
    certificateId,
    possessionAcknowledgedAt: acknowledgedAt.toISOString(),
  });
}
