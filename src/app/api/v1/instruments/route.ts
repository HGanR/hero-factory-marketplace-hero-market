/**
 * Platform API v1 - Instruments
 * GET /api/v1/instruments?trustId=... - List instruments (optionally filtered by trust)
 */
...
export async function GET(req: NextRequest) {
  ...
  const rows = await db
    ...
    .where(inArray(trustInstruments.trustId, trustIds))
    .limit(100);

  return NextResponse.json({
    data: rows.map((r) => serializeInstrument(r as unknown as Record<string, unknown>)),
