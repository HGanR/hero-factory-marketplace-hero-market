...
/**
 * LEENA — floating salon demo assistant (same footprint as MAANIA on the realtor demo).
 * Uses `NEXT_PUBLIC_SALON_DEMO_WIDGET_KEY` so it can bind to a separate Agency widget from RET/MAANIA.
 */
...
const AGENT_NAME = process.env.NEXT_PUBLIC_SALON_AGENT_NAME?.trim() || "LEENA";
const AVATAR = "/for-salon-professionals/leena-salon-agent.png";

const BRAND = "#7C3AED";
const BRAND_GLOW = "0 0 20px rgba(124,58,237,0.45), 0 0 40px rgba(236,72,153,0.15)";
...
function renderBold(text: string): ReactNode {
  ...
  while ((match = boldRegex.exec(text)) !== null) {
    ...
    parts.push(
      <strong key={`b${k++}`} className="font-semibold text-violet-200">
        {match[1]}
      </strong>
    );
...
const WELCOME_MARKDOWN = `Hi — I'm **${AGENT_NAME}**, your Studio North assistant. Ask about services, booking, policies, or what to expect before your visit.`;

export function SalonLeenaChat() {
  const widgetKey = process.env.NEXT_PUBLIC_SALON_DEMO_WIDGET_KEY?.trim();
  ...
  const [widgetSessionId] = useState(
    () =>
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `leena_${Date.now()}_${Math.random().toString(16).slice(2)}`
  );
    ...
    async (text: string) => {
      ...
      try {
        ...
        const context: Record<string, unknown> = {
          pageType: "salon",
          source: "salon_demo_leena",
          siteSection: "studio-north-demo",
          salonDemo: { brandLabel: "Studio North", assistant: AGENT_NAME },
        };
        ...
        return [
          {
            id: "welcome-setup",
            role: "assistant",
            content: `**Setup:** Add \`NEXT_PUBLIC_SALON_DEMO_WIDGET_KEY\` to your environment (generate a **separate** widget key in AI Agency for the salon agent so it does not conflict with RET). Allow **this site's origin** in that widget's allowed domains. You can add knowledge and refine the agent in Agency before going live.`,
          },
        ];
      }
      ...
      {!open && (
        ...
        >
          ...
          <div className="relative">
            ...
            />
            <div
              className="relative h-32 w-32 cursor-pointer overflow-hidden rounded-full border-4 transition-transform group-hover:scale-105 bg-white"
              style={{
                borderColor: BRAND,
                boxShadow: BRAND_GLOW,
              }}
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              <div className="relative h-11 w-11 overflow-hidden rounded-full border-2 bg-white" style={{ borderColor: BRAND }}>
              ...
              <div>
                ...
                <p className="text-[11px] text-pink-300/90">● Booking &amp; guest care</p>
              </div>
            </div>
            <button
              type="button"
            ...
            {messages.map((m) => (
              <div
                ...
                className={`max-w-[92%] rounded-xl px-3 py-2 text-[13px] leading-snug ${
                  m.role === "user"
                    ? "ml-auto border border-violet-500/30 bg-violet-600/20 text-slate-100"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  widgetKey
                    ? `Message ${AGENT_NAME}…`
                    : "Set NEXT_PUBLIC_SALON_DEMO_WIDGET_KEY to enable chat…"
                }
              ...
              <button
                ...
                style={{ background: `linear-gradient(135deg, ${BRAND}, #EC4899)` }}
                aria-label="Send"
              >
