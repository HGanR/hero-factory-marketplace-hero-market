/**
 * WorldObjects.tsx
 * Seven placeable environment objects for the Troo World editor.
...
export function ParkingLot({ position, rotation = [0, 0, 0], isSelected, onSelect }: WorldObjectProps) {
  ...
  return (
    <group position={position} rotation={rotation} onClick={onSelect}>
      ...
      {stalls}
      {[-1, 1].map((side, i) => (
        <mesh key={`h${i}`} position={[0, 0.1, side * 8.15]} castShadow>
          <boxGeometry args={[18, 0.2, 0.3]} />
          <meshLambertMaterial color={0xbbbbcc} />
        </mesh>
      ))}
      {[-1, 1].map((side, i) => (
        <mesh key={`v${i}`} position={[side * 9.15, 0.1, 0]} castShadow>
...
export function WorldObjectRenderer({
  ...
  switch (type) {
    ...
    case "bench":
      return <Bench {...props} />;
    case "lightpost":
      return <LightPost {...props} />;
    case "parkinglot":
      return <ParkingLot {...props} />;
  }
}
