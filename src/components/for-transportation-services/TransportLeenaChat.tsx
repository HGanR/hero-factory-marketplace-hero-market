...
/**
 * Transportation demo assistant — widget pattern aligned with other verticals.
 * `NEXT_PUBLIC_TRANSPORT_DEMO_WIDGET_KEY` for Agency binding.
 */
...
const AGENT_NAME = process.env.NEXT_PUBLIC_TRANSPORT_AGENT_NAME?.trim() || "DERRON";
const AVATAR = "/transport-derron-avatar.png";
...
const WELCOME_MARKDOWN = `Hi — I'm **${AGENT_NAME}**, your booking assistant. Ask about airport transfers, hourly chauffeur blocks, events, or what to expect before your ride.`;

export function TransportLeenaChat() {
  const widgetKey = process.env.NEXT_PUBLIC_TRANSPORT_DEMO_WIDGET_KEY?.trim();
    ...
    async (text: string) => {
      ...
      try {
        ...
        const context: Record<string, unknown> = {
          pageType: "transport",
          source: "transportation_demo",
          siteSection: "transport-demo",
          transportDemo: { assistant: AGENT_NAME },
        };
        ...
        return [
          {
            id: "welcome-setup",
            role: "assistant",
            content: `**Setup:** Add \`NEXT_PUBLIC_TRANSPORT_DEMO_WIDGET_KEY\` to your environment (separate Agency widget for transportation). Allow **this site's origin** in that widget's allowed domains.`,
          },
        ];
      }
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-emerald-400/90">● Booking &amp; trip intake</p>
              </div>
            </div>
            <button
              type="button"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  widgetKey
                    ? `Message ${AGENT_NAME}…`
                    : "Set NEXT_PUBLIC_TRANSPORT_DEMO_WIDGET_KEY to enable chat…"
                }
