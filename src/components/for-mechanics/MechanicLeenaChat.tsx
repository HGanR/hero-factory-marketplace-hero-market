...
/**
 * Mechanics / autobody demo assistant — `NEXT_PUBLIC_MECHANICS_DEMO_WIDGET_KEY` for Agency binding.
 */
...
const AGENT_NAME = process.env.NEXT_PUBLIC_MECHANICS_AGENT_NAME?.trim() || "TAO";
const AVATAR = "/mechanics-tao-avatar.png";

const BRAND = "#94a3b8";
const BRAND_GLOW = "0 0 20px rgba(148,163,184,0.35), 0 0 40px rgba(71,85,105,0.2)";
...
function renderBold(text: string): ReactNode {
  ...
  while ((match = boldRegex.exec(text)) !== null) {
    ...
    parts.push(
      <strong key={`b${k++}`} className="font-semibold text-slate-200">
        {match[1]}
      </strong>
    );
...
const WELCOME_MARKDOWN = `Hi — I'm **${AGENT_NAME}**, your shop assistant. Ask about services, estimates, drop-off, or repair status — I'm here to help.`;

export function MechanicLeenaChat() {
  const widgetKey = process.env.NEXT_PUBLIC_MECHANICS_DEMO_WIDGET_KEY?.trim();
  ...
  const [widgetSessionId] = useState(
    () =>
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `mc_${Date.now()}_${Math.random().toString(16).slice(2)}`
  );
    ...
    async (text: string) => {
      ...
      try {
        ...
        const context: Record<string, unknown> = {
          pageType: "mechanics",
          source: "mechanics_demo",
          siteSection: "mechanics-demo",
          mechanicsDemo: { assistant: AGENT_NAME },
        };
        ...
        return [
          {
            id: "welcome-setup",
            role: "assistant",
            content: `**Setup:** Add \`NEXT_PUBLIC_MECHANICS_DEMO_WIDGET_KEY\` to your environment (separate Agency widget for mechanics / autobody). Allow **this site's origin** in that widget's allowed domains.`,
          },
        ];
      }
    ...
    <>
        ...
        >
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{ backgroundColor: "#475569", boxShadow: BRAND_GLOW }}
            >
              AI
            </div>
          </div>
      ...
      {open && (
        <div
          ...
          style={{
            background: "linear-gradient(145deg, rgba(15,23,42,0.98) 0%, rgba(15,23,42,0.99) 100%)",
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">● Estimates, booking &amp; status</p>
              </div>
            </div>
            <button
              type="button"
            ...
            {messages.map((m) => (
              <div
                ...
                className={`max-w-[92%] rounded-xl px-3 py-2 text-[13px] leading-snug ${
                  m.role === "user"
                    ? "ml-auto border border-slate-500/35 bg-slate-800/60 text-slate-100"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  widgetKey
                    ? `Message ${AGENT_NAME}…`
                    : "Set NEXT_PUBLIC_MECHANICS_DEMO_WIDGET_KEY to enable chat…"
                }
              ...
              <button
                ...
                style={{ background: `linear-gradient(135deg, ${BRAND}, #475569)` }}
                aria-label="Send"
              >
