...
/**
 * Barbershop demo assistant — widget pattern aligned with other verticals.
 * `NEXT_PUBLIC_BARBERSHOP_DEMO_WIDGET_KEY` for Agency binding.
 */
...
const AGENT_NAME = process.env.NEXT_PUBLIC_BARBERSHOP_AGENT_NAME?.trim() || "REN";
const AVATAR = "/barbershop-ren-avatar.png";
...
const WELCOME_MARKDOWN = `Hi — I'm **${AGENT_NAME}**, your shop assistant. Ask about cuts, booking, hours, or promos — even when the team's behind the chair.`;

export function BarbershopLeenaChat() {
  const widgetKey = process.env.NEXT_PUBLIC_BARBERSHOP_DEMO_WIDGET_KEY?.trim();
  ...
  const [widgetSessionId] = useState(
    () =>
      typeof crypto !== "undefined" && crypto.randomUUID
        ? crypto.randomUUID()
        : `br_${Date.now()}_${Math.random().toString(16).slice(2)}`
  );
    ...
    async (text: string) => {
      ...
      try {
        ...
        const context: Record<string, unknown> = {
          pageType: "barbershop",
          source: "barbershop_demo",
          siteSection: "barbershop-demo",
          barbershopDemo: { assistant: AGENT_NAME },
        };
        ...
        return [
          {
            id: "welcome-setup",
            role: "assistant",
            content: `**Setup:** Add \`NEXT_PUBLIC_BARBERSHOP_DEMO_WIDGET_KEY\` to your environment (separate Agency widget for barbershops). Allow **this site's origin** in that widget's allowed domains.`,
          },
        ];
      }
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-400/90">● Shop assistant — booking &amp; guest care</p>
              </div>
            </div>
            <button
              type="button"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  widgetKey
                    ? `Message ${AGENT_NAME}…`
                    : "Set NEXT_PUBLIC_BARBERSHOP_DEMO_WIDGET_KEY to enable chat…"
                }
