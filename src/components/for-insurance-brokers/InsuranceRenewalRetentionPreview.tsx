      ...
      <div className="space-y-3 p-4 text-[13px] leading-relaxed">
        <div className="flex justify-end">
          <div className="max-w-[90%] rounded-2xl rounded-tr-md border border-white/10 bg-slate-800/90 px-4 py-3 text-slate-100">
            ...
            <p className="mt-1">I want to review my current policy before renewal.</p>
          </div>
        </div>
        <div className="flex justify-start">
          <div className="max-w-[95%] rounded-2xl rounded-tl-md border border-cyan-500/25 bg-cyan-950/35 px-4 py-3 text-slate-100">
            <p className="text-[10px] font-semibold uppercase text-cyan-300/90">Assistant</p>
            <p className="mt-2">What type of policy is it?</p>
          </div>
        </div>
        <div className="flex justify-end">
          <div className="max-w-[85%] rounded-2xl rounded-tr-md border border-white/10 bg-slate-800/90 px-4 py-2.5">
            <p className="text-[10px] text-slate-500">Client</p>
            <p className="mt-1">Homeowners</p>
          </div>
        </div>
        <div className="flex justify-start">
          <div className="max-w-[95%] rounded-2xl rounded-tl-md border border-cyan-500/20 bg-slate-900/80 px-4 py-3 text-slate-200">
            ...
            <p className="mt-2">Would you like a broker to review coverage, premium changes, or both?</p>
          </div>
        </div>
        <div className="flex justify-end">
          <div className="max-w-[85%] rounded-2xl rounded-tr-md border border-white/10 bg-slate-800/90 px-4 py-2.5">
        ...
        <div className="flex justify-start">
          <div className="max-w-[95%] rounded-2xl rounded-tl-md border border-emerald-500/30 bg-emerald-950/35 px-4 py-3 text-slate-100">
            <p className="text-[10px] font-semibold uppercase text-emerald-300/90">Assistant</p>
            <p className="mt-2">Great — we can route this for follow-up.</p>
          </div>
        </div>
      </div>
    </div>
...
export function InsuranceRenewalRetentionPreview() {
  const bullets = [
    "Routes renewal questions and service requests clearly",
    "Supports policy review prompts before renewal dates",
    "Helps prevent lost accounts due to communication gaps",
    "Encourages consistent follow-up rhythms",
    "Improves client confidence and retention",
  ];

  return (
    <section className="border-t border-slate-800/80 bg-[#07111a] px-4 py-16 md:py-20">
      <div className="mx-auto max-w-6xl">
        <p className="text-center text-[11px] font-bold uppercase tracking-[0.35em] text-emerald-300/80 md:text-left">
          Revenue Is Also What You Keep
        </p>
        <h2 className="mt-3 text-center text-2xl font-bold leading-tight text-white md:text-left md:text-3xl">
          Renewals, reminders, and service touchpoints built into the flow.
        </h2>
        <p className="mx-auto mt-4 max-w-3xl text-center text-slate-300 md:mx-0 md:text-left md:text-lg">
          Retention is a process — not a hope — when follow-ups have a clear path.
        </p>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-2 lg:items-start lg:gap-14">
          <div className="space-y-6">
            ...
            <p className="text-[13px] leading-relaxed text-slate-500">
              <span className="font-semibold text-slate-400">Production note.</span> Brokerage-configured workflow.
              Reminder timing and retention processes vary by agency operations and applicable requirements.
            </p>
          </div>
          <div>
            <p className="mb-4 text-center text-[11px] font-medium uppercase tracking-wider text-slate-500 lg:text-left">
              View-only renewal example
            </p>
            <RenewalMock />
          </div>
        </div>
