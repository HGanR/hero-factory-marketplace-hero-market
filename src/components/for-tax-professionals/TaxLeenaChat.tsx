...
/**
 * Tax professional demo assistant — widget pattern aligned with other verticals.
 * `NEXT_PUBLIC_TAX_DEMO_WIDGET_KEY` for Agency binding.
 */
...
import taxAgentAvatar from "./tax-agent-chas.png";

/** Display name for the tax demo assistant (bundled; not overridden by env so stale Vercel keys can't show “Ledger”). */
const AGENT_NAME = "Chas";

const BRAND = "#d97706";
const BRAND_GLOW = "0 0 20px rgba(217,119,6,0.35), 0 0 40px rgba(120,113,108,0.12)";
...
function renderBold(text: string): ReactNode {
  ...
  while ((match = boldRegex.exec(text)) !== null) {
    ...
    parts.push(
      <strong key={`b${k++}`} className="font-semibold text-amber-200">
        {match[1]}
      </strong>
    );
...
const WELCOME_MARKDOWN = `Hi — I'm **${AGENT_NAME}**, your intake assistant. Ask about services, what to bring, or how to start your request — I'm here to guide you.`;

export function TaxLeenaChat() {
  const widgetKey = process.env.NEXT_PUBLIC_TAX_DEMO_WIDGET_KEY?.trim();
    ...
    async (text: string) => {
      ...
      try {
        ...
        const context: Record<string, unknown> = {
          pageType: "tax",
          source: "tax_demo",
          siteSection: "tax-demo",
          taxDemo: { assistant: AGENT_NAME },
        };
        ...
        return [
          {
            id: "welcome-setup",
            role: "assistant",
            content: `**Setup:** Add \`NEXT_PUBLIC_TAX_DEMO_WIDGET_KEY\` to your environment (separate Agency widget for tax professionals). Allow **this site's origin** in that widget's allowed domains.`,
          },
        ];
      }
    ...
    <>
      {!open && (
        ...
        >
          ...
          <div className="relative">
            ...
            >
              <Image src={taxAgentAvatar} alt={AGENT_NAME} fill className="object-cover object-top" sizes="128px" priority />
            </div>
            <div
      ...
      {open && (
        <div
          ...
          style={{
            background: "linear-gradient(145deg, rgba(12,10,8,0.98) 0%, rgba(8,7,6,0.99) 100%)",
          ...
          >
            <div className="flex items-center gap-2">
              <div className="relative h-11 w-11 overflow-hidden rounded-full border-2 bg-slate-900" style={{ borderColor: BRAND }}>
                <Image src={taxAgentAvatar} alt={AGENT_NAME} fill className="object-cover object-top" sizes="44px" />
              </div>
              <div>
                <h3 className="text-sm font-bold tracking-wide text-white">{AGENT_NAME}</h3>
                <p className="text-[11px] text-amber-400/90">● Tax intake &amp; scheduling</p>
              </div>
            </div>
            <button
              type="button"
            ...
            {messages.map((m) => (
              <div
                ...
                className={`max-w-[92%] rounded-xl px-3 py-2 text-[13px] leading-snug ${
                  m.role === "user"
                    ? "ml-auto border border-amber-500/30 bg-amber-950/40 text-slate-100"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  widgetKey
                    ? `Message ${AGENT_NAME}…`
                    : "Set NEXT_PUBLIC_TAX_DEMO_WIDGET_KEY to enable chat…"
                }
              ...
              <button
                ...
                style={{ background: `linear-gradient(135deg, ${BRAND}, #78716c)` }}
                aria-label="Send"
              >
